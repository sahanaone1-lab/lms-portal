...
import { ProjectSubmissionStatus, Role } from '@prisma/client';
...
export class ProjectSubmissionsService {
   ...
   */
  private async findCoordinatorByDomain(domain: string): Promise<string> {
    ...
    if (
      courseWithCoord?.projectCoordinator &&
      courseWithCoord.projectCoordinator.role === Role.PROJECT_COORDINATOR
    ) {
    ...
    if (anyCoordinator) {
      this.logger.warn(
        `No coordinator found for domain "${domain}", falling back to first available coordinator`,
      );
      return anyCoordinator.id;
    }

    throw new NotFoundException(
      `No Project Coordinator found for domain "${domain}". Please contact an administrator.`,
    );
  }
  ...
  async uploadAndSave(
    ...
    if (student.role !== Role.INTERN) {
      throw new ForbiddenException('Only interns can submit projects');
    }

    const internDomain = student.domain;
    if (!internDomain) {
      throw new BadRequestException(
        'Your account does not have a domain assigned. Please contact an administrator.',
      );
    }
    ...
    if (projectId) {
      ...
      if (project.domain.toLowerCase() !== internDomain.toLowerCase()) {
        throw new ForbiddenException('You can only submit for projects in your domain');
      }
    }
    ...
    try {
      ...
      throw new InternalServerErrorException('File upload to S3 failed. Please try again.');
    }
    ...
    try {
      ...
      throw new InternalServerErrorException(
        'File uploaded to S3 but database save failed. Please contact support.',
      );
    }
  }
      ...
      return this.prisma.projectSubmission.findMany({
        where: {
          OR: [
            ...
            ...(myDomain
              ? [{ domain: { equals: myDomain, mode: 'insensitive' as const } }]
              : []),
          ],
        },
        include: {
  ...
  async review(
    ...
    if (!['SUBMITTED', 'UNDER_REVIEW', 'APPROVED', 'REJECTED'].includes(status)) {
      throw new BadRequestException(
        'Invalid status. Must be SUBMITTED, UNDER_REVIEW, APPROVED, or REJECTED',
      );
    }
   ...
   */
  async findOne(submissionId: string, userId: string, role: Role) {
    const submission = await this.prisma.projectSubmission.findUnique({
      where: { id: submissionId },
      include: {
        student: { select: { name: true, email: true } },
    ...
    if (role === Role.PROJECT_COORDINATOR) {
      ...
      const hasAccess =
        submission.projectCoordinatorId === userId ||
        (coordinator?.domain &&
          submission.domain.toLowerCase() === coordinator.domain.toLowerCase());
