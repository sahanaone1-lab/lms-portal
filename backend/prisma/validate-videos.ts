...
import { YoutubeService } from '../src/lessons/youtube.service';
...
async function ensureFullStackCourse(projectCoordinatorId: string) {
  const fsDomain = 'Full Stack';
  
  // 1. Check if course exists for Full Stack domain
  ...
  if (!course) {
    ...
    // Create weeks/modules JSON structure
    const weeks = [
...
async function main() {
  ...
  // Get a project coordinator for fallback
  ...
  // Ensure Full Stack Development course exists
