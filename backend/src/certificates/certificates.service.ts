...
export class CertificatesService {
  ...
  async getMyCertificates(studentId: string) {
    ...
    const pendingRejected = requests.map((r: any) => ({
      id: r.id,
      certificateCode: '',
  ...
  async claim(studentId: string, courseId: string) {
    // 1. Verify enrollment
    ...
    if (existingReq) {
      if (existingReq.status === 'APPROVED') {
        throw new BadRequestException('Certificate has already been approved and issued.');
      }
      ...
      if (isProject) {
        if (!submission) {
          throw new BadRequestException(
            `Cannot claim certificate. Please submit project: ${assignment.title}`,
          );
        }
        if (!submission.isApproved) {
          throw new BadRequestException(
            `Cannot claim certificate. Project "${assignment.title}" is pending project coordinator approval.`,
          );
        }
      } else {
        ...
        if (!submission) {
          throw new BadRequestException(
            `Cannot claim certificate. Please submit assignment: ${assignment.title}`,
          );
        }
      }
    }
    ...
    for (const quiz of quizzes) {
      ...
      if (!passedResult) {
        throw new BadRequestException(
          `Cannot claim certificate. Please pass quiz: ${quiz.title}`,
        );
      }
    }
  ...
  async getDomainCertificateRequests(projectCoordinatorId: string) {
  ...
  async approveRequest(requestId: string) {
    ...
    if (!req) throw new NotFoundException('Certificate request not found');
