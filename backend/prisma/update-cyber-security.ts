...
// ─── Cyber Security — 120 Hours Curriculum ────────────────────────────────────
...
async function updateCyberSecurityCourse() {
  ...
  // 2. Update course metadata with 10 modules
  await prisma.course.update({
    where: { id: course.id },
    data: {
      title: 'Cyber Security Training Program',
      description:
        'Industry-Oriented Cyber Security Course — 120 Hours. Master Networking, Ethical Hacking, Penetration Testing, Web Application Security, SOC Operations, Digital Forensics, Cloud Security, and Security Automation with real lab environments and hands-on practicals.',
      duration: '120 Hours',
      weeks: csModules as any,
      status: 'Published',
    },
  });
  console.log(`  ✅ Updated ${csModules.length} modules`);

  // 3. Create lessons (one detailed lesson per module)
  ...
  await prisma.quiz.create({
    data: {
      ...
      courseId: course.id,
      weekId: 'cs_m10',
      questions: csQuizQuestions as any,
    },
  });
